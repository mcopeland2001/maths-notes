...
\searchableSection{Vector Spaces}{vector spaces, polynomials, periodic functions}
	...
	\searchableSubsection{Definition of a Vector Space}{vector spaces}{
		...
		\bigskip\bigskip
		\subsubsubsection{Isomorphisms between vector spaces}
...
\searchableSubsection{Finite Sets of Vectors}{vector spaces}{	
	...
	\medskip
	\labeledProposition{Let ${ W_1, \dots, W_n }$ be subspaces of a finite-dimensional vector space $V$, and let $B_i$ be a basis for $W_i$. Then, the ordered set $B$ obtained by listing the bases ${ B_1, \dots, B_n }$ in order is a basis of $V$ iff $V$ is the direct sum ${ W_1 \otimes \cdots \otimes W_n }$.
	}{properties_of_direct_sum_of_finite_vector_space}
	\begin{proof}
	...
	\medskip
	\labeledProposition{Let $W$ be a subspace of a finite-dimensional vector space $V$. Then there is another subspace $W'$ such that ${ W \oplus W' = V }$.}{subspaces_have_a_complement_wrt_direct_sum}
	\begin{proof}
	...
	\end{proof}
	\note{The subspace $W'$ of $V$ such that ${ W \oplus W' = V }$ is called the \textbf{quotient space of $V$ by $W$} and is denoted ${ V/W }$ (see \href{https://en.wikipedia.org/wiki/Quotient_space_(linear_algebra)}{wikipedia}).}
}
