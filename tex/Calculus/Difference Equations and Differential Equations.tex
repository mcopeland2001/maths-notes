...
% ------------------- 	
	...
	\searchableSubsection{Second-order Difference Equations}{difference equations}{
		...
		\TODO{eh? if divide by $i$ then the real part will be divided by $i$ also}(answer: \ref{prop:linear-span-of-conjugate-pair-vectors-is-span-of-real-and-imaginary-parts})
...
% -----------------	
	...
	\biggerskip
		...
		\begin{figure}
			...
			\subfloat[\raggedright autonomous: ${ y' = y (1-y) }$ - note that the field is invariant to a shift along the $x$-axis.]{\includegraphics[height=\figHeight]{\autonomousDir/autonomous.png}}
...
% ------------------ break -------
	...
	\pagebreak
	\searchableSubsection{Systems of ODEs}{differential equations}{
		\biggerskip
		\boxeddefinition{A \textbf{system} of differential equations is a collection of related differential equations that must be solved simultaneously.\\
		...
		\boxeddefinition{A \textbf{solution} to a system of differential equations means to have found an analytic expression for each of the dependent variables in the system in terms of the independent variable only. So, in the system,
			\begin{align*}
				\dod{x}{t} &= f_1(t,x,y) \nn
				\dod{y}{t} &= f_2(t,x,y). \\
			\end{align*}
		...
		For example, the system of two first-order equations (${ 2 \times 1 }$ system),
		\[
			\begin{aligned}
				\dod{x}{t} &= x + y^2 \nn
				\dod{y}{t} &= -x
			\end{aligned}
		\]
		can be transformed into one second-order equation (${ 1 \times 2 }$ system) by observing that ${ \tod{x}{t} = -\tod[2]{y}{t} }$ so that,
		\[
			\begin{aligned}
				&& \dod{x}{t} &= x + y^2 \nn
				&\iff & -\dod[2]{y}{t} &= -\dod{y}{t} + y^2 \nn
				&\iff & \dod[2]{y}{t} &= \dod{y}{t} - y^2. \nn
			\end{aligned}
		\]
...
%---------- 
		...
		\boxeddefinition{A \textbf{vector-valued function} can be viewed either as a function that returns a vector or as a vector whose entries are functions. These views are equivalent because we can view each element of the vector as being generated by a separate function (branch) within the function.\\
			
			For example, let ${ X: \R{} \longmapsto \R{2} }$. Then we can represent $X(t)$ as
			\[ X(t) = \x \]
			for ${ \x \in \R{2} }$, or as
			\[ \begin{bmatrix}
				x_1(t)\\
			...
			Similarly, a \textbf{matrix-valued function} ${ A: \R{} \longmapsto \R{2 \times 2} }$ can be represented as
			\[ A(t) = A \]
			for ${ A \in \R{2 \times 2} }$ or by
			\[ \begin{bmatrix}
				a_{11}(t) & a_{12}(t)\\
				a_{21}(t) & a_{22}(t)
			\end{bmatrix}. \]
			...
			\[ \lim_{t \to a} A(t) = 
						\begin{bmatrix}
							\lim_{t \to a} a_{11}(t) & \lim_{t \to a} a_{12}(t)\\
							\lim_{t \to a} a_{21}(t) & \lim_{t \to a} a_{22}(t)
						\end{bmatrix} 
			\]
			and
			\[ \dod{A(t)}{t} = 
						\begin{bmatrix}
							\dod{}{t} a_{11}(t) & \dod{}{t} a_{12}(t)\nnn
							\dod{}{t} a_{21}(t) & \dod{}{t} a_{22}(t)
						\end{bmatrix}.
			\]
		}	
		\clearpage
		
		\subsubsubsection{Eigenvalues and Eigenvectors}
		
		\begin{tcolorbox}[breakable,enhanced jigsaw,colframe=white,colback=white,boxrule=0pt,arc=0pt,left=0pt,right=0pt,top=0pt,bottom=0pt]
			\labeledProposition{Let $A$ and $E$ be ${ n \times n }$ matrices such that the columns of $E$ are an eigenbasis of $A$ and ${ D = \inv{E}AE }$ is diagonal. Then \textbf{all} the solutions of the system of equations,
				\[ \dod{X(t)}{t} = AX(t) \]
				have the form ${ X(t) = c_1 \v_1 e^{\lambda_1 t} + c_2 \v_2 e^{\lambda_2 t} + \cdots + c_n \v_n e^{\lambda_n t} }$ where ${ \setc{\lambda_i}{1 \leq i \leq n} }$ are the eigenvalues along the diagonal of the matrix $D$ and ${ \setc{\v_i}{1 \leq i \leq n} }$ are the corresponding eigenvectors.
			}{eigenvectors-eigenvalues-are-solutions-to-system-linear-diff-eqs}
			\begin{proof}
		...
		\note{If $\beta$ is a function of $t$ then we would have,
			\[ \dod{}{t}(\beta(t) e^{\int \lambda \dif t}) = \lambda(t) \beta(t) e^{\int \lambda \dif t} + \beta'(t) e^{\int \lambda \dif t}. \]
			This leads to the formula for the solutions of non-separable linear differential equations,
			\[ y' = f(t)y + g(t) \implies y(t) = \left( \int g(t) e^{-\int f(t) \dif t} \dif t \right) e^{\int f(t) \dif t} \]
			where ${ \lambda(t) = f(t) }$.
		}
		...
		\bigskip
		\subsubsubsection{The Matrix Exponential}
		\TODO{proofs of matrix exponential propositions}
		\TODO{do these only apply to constant-coefficient equations?}
		
		\biggerskip
		\subsubsubsection{Complex Eigenvalues}
		...
		\begin{exe}
			\ex{
				...
				\[\begin{aligned}
					...
					&=  \begin{bmatrix}
						c_1 i (\cos{t} + i \sin{t}) - c_2 i (\cos{t} - i \sin{t}) \\
						c_i (\cos{t} + i \sin{t}) + c_2 (\cos{t} - i \sin{t})
					\end{bmatrix} \nnn
					&=  \begin{bmatrix}
						c_1 i \cos{t} - c_1 \sin{t} - c_2 i \cos{t} - c_2 \sin{t} \\
						c_i \cos{t} + c_1 i \sin{t} + c_2 \cos{t} - c_2 i \sin{t}
					\end{bmatrix} \nnn
					&=  \begin{bmatrix}
						(c_1 - c_2) i \cos{t} - (c_1 + c_2) \sin{t} \\
						(c_1 + c_2) \cos{t} + (c_1 - c_2) i \sin{t}.
					\end{bmatrix} \nnn
				\end{aligned}\]
				If we have the initial conditions ${ \x(0) = \langle 1, 0 \rangle }$ then,
				\[\begin{aligned}
					&& \begin{bmatrix}1\\ 0\end{bmatrix} &= 	
					\begin{bmatrix}
						(c_1 - c_2) i \cos{0} - (c_1 + c_2) \sin{0} \\
						(c_1 + c_2) \cos{0} + (c_1 - c_2) i \sin{0}
					\end{bmatrix} \nnn
					&&  	&=	\begin{bmatrix}
						(c_1 - c_2) i \\
						(c_1 + c_2)
				...
				\[\begin{aligned}
					...
					&\implies & c_1 - c_2 = -c_2 - c_2 &= -2c_2 = -i \nn
					&\implies & c_2 &= \frac{i}{2} \nn
					&\implies & c_1 &= -\frac{i}{2}. \nn
				\end{aligned}\]
		...
		\biggerskip
		\subsubsubsection{Repeated Eigenvalues}
		...
		\biggerskip
		\subsubsubsection{Solving for the Non-homogeneous System}
		...
		\note{If the external driver signal is equal to the natural response of the system then we have
			\[ Dy = \lambda(t) y + g(t) \implies (D - \lambda(t)) \, \alpha(t) \, e^{\int \lambda(t) \dif t} = g(t) \]
			but with ${ g(t) = e^{\int \lambda(t) \dif t} }$. In this case,
			\[ D\alpha(t) = g(t) e^{-\int \lambda(t) \dif t} = 1 \implies \alpha(t) = t + C. \]
			This is \textbf{resonance}.
		}
	}
