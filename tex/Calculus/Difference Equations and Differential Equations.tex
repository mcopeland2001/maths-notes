...
% ----------------------------------
	...
	\searchableSubsection{Differential Equations}{differential equations}{
		...
		\note{Stable steady-states happen when greater values of the function decrease to the steady-state value and lesser values of the function increase to the steady-state value. An \textbf{unstable} steady-state, conversely, is one where values of the function on lesser and greater move away from the steady-state value; the states of real-world systems modelled by these steady-states may not, in practice, be referred to as equilibria due to their instability.\\\\
		Examples:
		\begin{itemize}
			\item{A ball rolling into a dip in the ground stays there whereas a ball at the top of a hump in the ground will likely roll off.}
			\item{The equation ${ y' = \sin(y) }$ in figure \ref{fig:Dy_eq_sin_y} shows a stable equilibrium at ${ y = \pi }$ and unstable equilibria at ${ y = 0, 2\pi }$.}
			\item{Another example: The equation ${ y' = \frac{4t}{y} - 2t }$ has a \textbf{stable} steady-state at ${ y = 2 }$ but the equation ${ y' = -\frac{4t}{y} + 2t }$ has an \textbf{unstable} steady-state at the same value ${ y = 2 }$. (Is this because for the first equation, at ${ y = y_s }$ we have ${ \tpd[2]{f}{y}\big\rvert_{y=y_s} > 0 }$ meaning that it is some kind of minimum? and the converse for the second equation?)
			}
		\end{itemize}
		}
