...
\begin{document}
	...
	\searchableSubsection{Relations}{Discrete Maths}{
...
\pagebreak
	...
	\searchableSubsection{Functions}{Discrete Maths}{
		...
		\boxeddefinition{\textbf{(Monotonic Function)} Let $<$ be a total order on a set $X$ and let ${ f: X \longmapsto X }$. Then $f$ is described as \textit{strictly monotonically increasing} if, for any ${ x,y \in X }$,
			\[ x < y \iff f(x) < f(y) \]
		and \textit{strictly monotonically decreasing} if
			\[ x < y \iff f(x) > f(y). \]
		In both cases $f$ can be described as \textit{monotonic}.
		\label{def:monotonic-function}}
	
		\biggerskip
		\labeledProposition{If a function ${ f: \N{} \longmapsto \N{} }$ is strictly monotonically increasing then, for any ${ n \in \N{} }$,
			\[ f(n) \geq n. \]
		}{strictly-increasing-function-over-naturals-has-value-geq-argument}
		\begin{proof}
			Since, for all ${ n \in \N{} }$ we have ${ n \geq 1 }$, then it follows that ${ f(1) \geq 1 }$.	But by the monotonically increasing property (\ref{def:monotonic-function}), for all ${ a, b \in \N{} }$,
			\[ a < b \iff f(a) < f(b). \]
			So, inductively, if ${ f(n) \geq n }$ then
			\[ f(n+1) > f(n) \geq n \; \implies \; f(n+1) > n \; \implies \; f(n+1) \geq n+1. \qedhere \]
		\end{proof}
