...
\begin{document}	
	...
	\searchableSubsection{The Axiom of Choice}{set theory}{
		...
		\boxedaxiom{\textbf{The Axiom of Choice} says that if $A$ is a non-empty set of non-empty sets then there exists a choice function for $A$.}
		...
		\boxedaxiom{The \textbf{Well-Ordering Theorem} states that every set can be well-ordered.}
		...
		\bigskip
		\boxedaxiom{(Zorn's Lemma). Let $(P, \leq)$ be a non-empty partial order such that every totally ordered subset has an upper bound. Then $P$ has a maximal element.}
		...
		\TODO{
			\begin{itemize}
				\item{proof that every vector space has a basis}
				\item{outline proof of the impossibility of a countable basis of the reals as a vector space over the rationals (reference the indicator function for the reals)}
				\item{reference in section on infinite vector spaces - which requires including this in main document (perhaps in a section entitled "Fundamentals"?)}
			\end{itemize}
		}
