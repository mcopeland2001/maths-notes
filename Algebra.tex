...
\searchableChapter{Algebra}{abstract algebra, complex numbers}
	...
	\searchableSubsection{Homomorphisms}{abstract algebra}{\bigskip
		...
		\labeledTheorem{\textbf{Cayley's Theorem} states that: that every group $G$ is isomorphic to a subgroup of the symmetric group acting on $G$.}{cayleys_theorem}
		\begin{proof}
			\TODO{this proof from Abstract Maths 14.6}
		\end{proof}
	...
	\pagebreak
	\searchableSubsection{Products Groups and Quotient Groups}{abstract algebra}{
		...
		\begin{proof}
			...
			\begin{enumerate}[label=(\roman*)]
				...
				\item{Assume \WLOG that $K$ is a normal subgroup. Then for all ${ k \in K,g \in G,\, \inv{g}kg \in K }$ and, in particular, for ${ h \in H, \inv{h}kh \in K }$. Therefore,
					...
					\[ hk,h'k' \in HK \eqand hh'kk' \in HK \implies hkh'k' \in HK \]
					so that $HK$ is closed and for ${ hk \in HK }$,
					\[ \inv{h}\inv{k} \in HK \implies \inv{k}\inv{h} \in HK \]
					so that $HK$ also contains inverses.
				}
				\item{If ${ H \cap K = \{1\} }$ then the product map $p$ is injective and if ${ HK = G }$ then ${ im\,p = G }$ so $p$ is surjective also and, therefore, is a bijection between ${ H \times K}$ and ${ G }$.\\\\
					If also both $H$ and $K$ are normal subgroups then ${ HK = KH }$ and for all ${ h \in H,\,k \in K,\; \inv{k}hk \in H \eqand \inv{h}kh \in K }$. Now, we know that $p$ is injective so that 
