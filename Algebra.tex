...
\searchableChapter{Algebra}{abstract algebra, complex numbers}
	...
	\searchableSection{Abstract Algebra}{abstract algebra}
	...
	\searchableSubsection{Normal Subgroups and Centers}{abstract algebra}{
		...
		\medskip
		\note{This is really the point of normal subgroups: That their cosets contain multiplication from both sides. As a result, when the cosets themselves are used as members of groups (see: Quotient Groups), the resulting groups are Abelian.}
		\TODO{link to Quotient Groups}
		
		
		\bigskip
		\subsubsection{Examples of Normal Subgroups} \TODO{find an example of a non-Abelian normal subgroup}
		\begin{exe}
	...
	\bigskip\bigskip
	\searchableSubsection{Modular Arithmetic}{abstract algebra, modular arithmetic}{
		\TODO{Describe modular arithmetic in terms of cosets: Artin[79]}
	}

	
	\pagebreak
	\searchableSubsection{Product Groups and Quotient Groups}{abstract algebra}{
		\subsubsection{Product Groups}
		\boxeddefinition{Let ${ G,G' }$ be two groups. The \textbf{product group} is the set ${ G \times G' }$ with component-wise multiplication.}
		
		\notation{The \textbf{product group} of the two groups ${ G,G' }$ may be denoted by ${ G \times G' }$ or $GG'$.}
		
		So, if ${ a,b \in G }$ and ${ a', b' \in G' }$ then 
		\begin{itemize}
			\item{${ (a,a'), (b,b') \in G \times G' }$}
			\item{${ (a,a')(b,b') = (ab, a'b') }$}
			\item{the identity is ${ (1,1) }$ and ${ \inv{(a,a')} = (\inv{a}, \inv{a'}) }$.}
		\end{itemize}
	
		\boxeddefinition{The \textbf{projections} of a product group ${ G \times G' }$ are the maps $p,p'$ such that,
		...
		\begin{proof}
			...
			\[ \phi(x^i) = (y^i, z^i). \]
			Then $\phi$ is homomorphic because,
			\[ \phi(x^ix^j) = \phi(x^{i+j}) = (y^{i+j}, z^{i+j}) = (y^i, z^i)(y^j, z^j). \]
			And $\phi$ is injective because,
			\[ \phi(x^i) = \phi(x^j) \iff (y^i, z^i) = (y^j, z^j) \iff (y^i = y^j) \wedge (z^i = z^j) \]
			which requires that ${ \abs{i - j} }$ be a multiple of both $r$ and $s$. But $r$ and $s$ are coprime so their lowest common multiple is $rs$ which is the order of ${ x \in C }$ so we have,
			\[ \phi(x^i) = \phi(x^j) \iff x^i = x^j. \]
			Now, the order of the image of $\phi$ is the same as the order of $C$ and of ${ C_1 \times C_2 }$ and so it is a bijection and therefore isomorphic.
		\end{proof}
		...
		\TODO{finish notes on Product Groups: Artin[78]}
