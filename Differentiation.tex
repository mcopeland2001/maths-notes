...
%		\[ A\vtheta = (b,a,0,\dots,0)^T. \]
		...
		\[ y(x) = \alpha_0 e^x. \]
		\TODO{how does this relate to the shift operator describe in a note to \autoref{prop:properties_of_non_bijective_lin_operator}?}
...
% ------------
	...
	\pagebreak	
	\searchableSection{Differentiation of Homogeneous Functions}{analysis, differentiation, homogeneous functions}{
	\bigskip\bigskip
		...
		\begin{proof}
			...
			Therefore, for each term of $f(\lambda x_1, \dots, \lambda x_n)$ we have,
			\[ (\lambda x_1)^{i_1} \cdots (\lambda x_n)^{i_n} = \lambda^{i_1} x_1^{i_1} \cdots \lambda^{i_n} x_n^{i_n} = \lambda^{(i_1 + \cdots + i_n)} x_1^{i_1} \cdots x_n^{i_n}. \]
			...
			\[ \lambda^d x_1^{i_1} \cdots x_n^{i_n} \]
			...
			So, if we take the partial derivative of $f$ with respect to $x_1$, each term of the result will take the form,
			\[ i_1x_1^{(i_1 - 1)}x_2^{i_2}\cdots x_n^{i_n} \]
			and each term of the partial derivative with respect to $x_2$ will have the form,
			\[ i_2x_1^{i_1}x_2^{(i_2 - 1)}\cdots x_n^{i_n} \]
			and the $n$-th partial derivative will have terms,
			\[ i_nx_1^{i_1}x_2^{i_2}\cdots x_n^{(i_n - 1)}. \]
			Now if we look at the terms of the expression $x_1f_{x_1}$,
			\[ x \cdot i_1x_1^{(i_1 - 1)}x_2^{i_2}\cdots x_n^{i_n} = i_1x_1^{i_1}x_2^{i_2}\cdots x_n^{i_n} \]
			we see that the terms are the same as the terms of the partial derivative with respect to $x_1$ except multiplied by the power of $x_1$ in that term. Therefore, each term of the expression ${ x_1f_{x_1} + \cdots + x_nf_{x_n} }$ has the form,
			\[ (i_1 + i_2 + \cdots + i_n)x_1^{i_1}x_2^{i_2}\cdots x_n^{i_n} = d \cdot x_1^{i_1}x_2^{i_2}\cdots x_n^{i_n}. \]
			Therefore,
