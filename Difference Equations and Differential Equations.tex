...
% ----------------------------------
	...
	\searchableSubsection{Differential Equations}{differential equations}{
		...
		&= f(x)y + g(x).
		\end{align*}
		\note{\TODO{This is correct but means that ${ Ae^{F(x)} }$ \textbf{is} the solution because we are trying to look for the most \textbf{general} solution we can find.} Note that
